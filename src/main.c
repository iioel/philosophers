...
/*   Updated: 2021/12/14 23:14:18 by ycornamu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int init_forks(pthread_mutex_t **fork, int nb)
{
	int i;

	i = 0;
	*fork = ft_calloc(nb, sizeof(pthread_mutex_t));
	if (! *fork)
		return (1);
	while (i < nb)
	{
...
//		return (print_usage());
		return (1);
	if (init_forks(&(params.fork), params.nb_philo))
		return (1);
